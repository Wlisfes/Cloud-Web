...
export default class NodeSource extends Vue {
	...
	/**分类标签信息**/
	private async nodeSource(id: number) {
		try {
			const { code, data } = await nodeSource({ id })
	...
	/**创建标签**/
	private async nodeCreateSource() {
		try {
			this.loading = true
	...
	/**修改标签**/
	private async nodeUpdateSource() {
		try {
			this.loading = true
			const { form } = this.state
	...
	private onSubmit() {
		this.$refs.form.validate(async valid => {
			if (valid) {
				switch (this.active) {
					case 'create':
						this.nodeCreateSource()
						break
					case 'update':
						this.nodeUpdateSource()
						break
				}
			}
		})
